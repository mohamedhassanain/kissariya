    ...
    return (
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {["auth-sk-1", "auth-sk-2", "auth-sk-3"].map((key) => (
              <Skeleton key={key} className="h-16" />
            ))}
          </div>
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingCategory 
                ? "Modifiez le nom de votre catégorie ci-dessous." 
      ...
      <AccordionItem value={category.id} className="border-2 rounded-lg px-4 bg-card">
        ...
        <AccordionContent className="pb-4">
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <Button variant="outline" size="sm" onClick={onDelete} className="h-8 px-2">
            ...
            >
              ...
              <span className="text-xs">Sous-catégorie</span>
            </Button>
          </div>
              ...
              {subcategories.map((sub) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="lowercase">{sub.name}</span>
                  </div>
                  <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleOpenSubDialog(sub)}>
                      <Edit className="h-3.5 w-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setDeleteSubId(sub.id)}>
      ...
      <Dialog open={subDialogOpen} onOpenChange={setSubDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingSub 
                ? "Modifiez le nom de votre sous-catégorie ci-dessous." 
                : "Ajoutez une nouvelle sous-catégorie à cette catégorie."}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Input
